...
from dn_helper import create_tables_and_store_data
...
def choose_directory():
    global directory
    directory = filedialog.askdirectory(initialdir="/home/roman/Downloads/Resumes", title="Select directory with Resumes", mustexist=True)
    return
...
def config_frames(logger):
    ...
    frame2.pack()
    btn_choose_directory = tk.Button(frame1, text="Choose Directory", command=choose_directory)
    btn_choose_directory.pack(side=tk.LEFT)
    btn_start = tk.Button(frame1, text="Start", command=lambda: start_processing_thread(logger))
    btn_start.pack(side=tk.LEFT)
    txt_logs = tk.Text(frame2, height=40, width=150)
    txt_logs.pack()
